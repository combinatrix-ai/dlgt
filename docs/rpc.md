...
Claude Code the daemon converts a bare key to `--KEY` and a keyed value to
`--KEY=VALUE`. For Codex every entry must be `KEY=VALUE`; the daemon converts
it to `--config KEY=VALUE` on both the managed app-server and remote TUI
processes. The daemon rejects dlgt-managed arguments and retains the array so
`session.restart` reuses the same launch behavior. When the array carries no
`permission-mode` entry and `auto_approve` is true, dlgt adds
